...
window["Silica"] = {
  ...
  ignore(keys) {
    Silica.ignoredAttributes = new Set(
      [...Silica.ignoredAttributes].concat(keys)
    );
  },
    ...
    for (let i = nodes.length - 1; i >= 0; --i) {
      ...
      if (!node.dataset["siO2TemplateId"]) {
        ...
        if (node.children.length === 1) {
          Silica._repeat_templates[nextTemplateId] = node.removeChild(
        ...
        Silica._repeat_templates[nextTemplateId] = Silica.compile(
          Silica._repeat_templates[nextTemplateId],
          false,
          context,
          true,
          false
  ...
  getValue(raw, propString, context = null, params = []) {
    ...
    ctx =
      context ||
      (propString.charCodeAt(idx) <= 90 ? window : Silica.getContext(raw));
    let param;
    if (raw.nodeType !== 8) {
  ...
  setPropByString(obj, propString, value) {
    ...
    if (propString.charCodeAt(0) <= 90) {
      ...
      if (
        !obj.hasOwnProperty(paths[0]) &&
        typeof obj[paths[0]] !== "function" &&
        obj.$ctrl
      ) {
        ctx = obj.$ctrl;
      } else {
   ...
   */
  getContext(raw) {
    var ctrl, k, v, _ref, model, needsModel;
    let original = raw;
    /** @type {string} */
    var constructorName;
    ...
    while (true) {
      if (raw._rt_ctx) {
        ...
        raw.nodeType !== 3 &&
        raw.nodeType !== 8 &&
        raw.dataset["controller"]
      ) {
        constructorName = raw.dataset["controller"];
        if (
          typeof (_ref = constructorName.match(
            /((?:\w|\.)+)(?:\(([\w\.]+)\))*/
          ))[2] !== "undefined"
        ) {
          needsModel = true;
        ...
        constructor = /** @type {function(new:Object, !Element, ?Object=)} */ (eval(
          constructorName
        ));
        if (!constructor) {
          return console.error(
            "Unknown Controller: " + raw.dataset["controller"]
          );
        }
        ...
        original._rt_ctx = raw.parentElement._rt_ctx;
        raw = raw.parentElement;
      } else {
        original._rt_ctx = Silica.context;
  ...
  _call(element, evnt, act) {
    ...
    if (
      element !== evnt.target &&
      act === "click" &&
      (evnt.target.nodeName === "SELECT" || evnt.target.nodeName === "INPUT")
    ) {
      return;
    }
  ...
  getFilteredValue(raw, propString, elideFilterIf, paramsKeys = null) {
    ...
    [paramsKeys, needsShift] = paramsKeys
      ? [paramsKeys, false]
      : [propString.match("\\((.*)\\)"), true];
    ...
    if (filterKey && !Silica.filters[filterKey]) {
      throw new Error(
        "Unknown filter: '" + filterKey + "' for element: " + raw.outerHTML
      );
    }
  ...
  queryWithComments(root, ...attributes) {
    ...
    for (var i = comments.length - 1; i >= 0; --i) {
      ...
      if (node.nodeValue.charAt(0) === "<") {
        ...
        for (
          var j = attributes.length - 1, attr = attributes[j];
          j >= 0;
          attr = attributes[--j]
        ) {
